...
export interface OperationalGuidelinesOptions {
  ...
  /**
   * Absolute path to the user's per-project private memory file. See
...
function toolUsageRememberingFacts(
  ...
  - **User Project** (\`${options.userProjectMemoryPath}\`): Personal-to-the-user, project-specific notes that must **NOT** be committed to the repo (local dev-setup notes, personal reminders about this codebase, private workflows for this project). This file is private to the user and is loaded automatically alongside \`./GEMINI.md\` for this workspace only.`
      : '';
    return `
- **Memory Files:** You persist memories by editing \`GEMINI.md\` files directly with '${EDIT_TOOL_NAME}' or '${WRITE_FILE_TOOL_NAME}'. There is no \`save_memory\` tool. The current contents of all loaded \`GEMINI.md\` files are already in your context — do not re-read a file before editing it.
