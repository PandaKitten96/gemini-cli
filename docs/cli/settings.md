...
| Memory Manager                                       | `experimental.memoryManager`     | Disable the built-in save_memory tool and let the main agent persist memories by editing GEMINI.md files directly with edit/write_file. Routes facts to the project GEMINI.md, subdirectory GEMINI.md files, or the per-project user memory file (the global ~/.gemini/GEMINI.md is not modified). | `false` |
